...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        Returns:
            None
        """
        plt.title('Histogram for binomial data')
        plt.xlabel('value')
        plt.ylabel('count')
        pd_series = pd.DataFrame({'values': self.data})
        value_counts = pd_series['values'].value_counts()
    ...
    def pdf(self, k):
        ...
        return math.factorial(self.n) * (self.p ** k) * ((1 - self.p) ** (self.n - k)) / float(
    ...
    def __add__(self, other):
        ...
        """Function to add together two Binomial dist_pdf with equal p
        ...
        result.stdev = result.mean * (1 - result.p)
        return result
